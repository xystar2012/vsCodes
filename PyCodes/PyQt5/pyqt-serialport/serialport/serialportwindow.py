...
class SerialPortWindow(QtWidgets.QMainWindow,serialportform.Ui_SerialPortWindow):
    _receive_signal = QtCore.pyqtSignal(bytes)
    ...
    def initForms(self):
        ...
        self.textEditSent.installEventFilter(self)
            ...
            if e.type() == QtCore.QEvent.KeyPress:
                ...
                if re.match('[0-9a-fA-F]',chr(e.key())):
                    if len(self.textEditSent.toPlainText()) % 3 == 2:
                        self.textEditSent.insertPlainText(' ')
                    self.textEditSent.insertPlainText(chr(e.key()))
    ...
    def __save_recv_file__(self):
        filename = QtGui.QFileDialog.getSaveFileName(self, caption="Save Received File")
        try:
    ...
    def __display_recv_data__(self,data):
        if self.checkBoxDisplayHex.isChecked():
            ...
            self.textEditReceived.insertPlainText(data.decode('utf8'))
    ...
    def __set_send_hex__(self):
        ...
        if self.checkBoxSendHex.isChecked():
            text = self.textEditSent.toPlainText()
            hexstr = ' '.join('{:02x}'.format(ord(x)) for x in text) 
            # self.textEditSent.toPlainText.clear()
            self.textEditSent.setPlainText(hexstr)
        else:
            text = self.textEditSent.toPlainText()
            try:
                self.textEditSent.setPlainText(bytearray.fromhex(text).decode('UTF8'))
            except:
                self.textEditSent.clear()                
